    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (type === 'expense') {
          ...
          // Handle incomes
          const totalsBySource = result.reduce((acc, item) => {
            acc[item.source] = (acc[item.source] || 0) + item.amount;
            return acc;
          }, {});

          const uniqueSources = Object.keys(totalsBySource);
          const groupedData = uniqueSources.map(source => ({
            category: source,
            amount: totalsBySource[source],
          }));
